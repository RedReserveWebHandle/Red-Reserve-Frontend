...
const requestcard = ({ bloodGroup, units, date }) => {
  return (
    <div className="bg-blue-50 rounded-xl p-4 shadow-md flex flex-col items-start text-left">
      <h3 className="text-3xl font-bold text-blue-900 mb-1">{bloodGroup}</h3>
      <p className="text-gray-700 font-semibold mb-2">{units}</p>
      <div className="flex items-center space-x-2 text-blue-600 text-sm mb-4">
        {/* Calendar icon */}
        <svg
      ...
      <button className="bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-4 rounded-xl transition duration-300 ease-in-out self-end">
        Details
      </button>
    </div>
  )
